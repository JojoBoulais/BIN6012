...
def main(argv):
    ...
    for d, in_filepath in enumerate([argv.normal_h5ad, argv.disease_h5ad]):
        ...
        for p in list(pathways):
            ...
            out_filepath = "_".join(
                [str(Path(adata_filepath).stem), disease, p])
            ...
            diff = list(set(genes).difference(
                set(grn_for_plot.available_genes)))
            ...
            mat = grn_for_plot.grn.loc[genes, genes].rename(
                index=rn, columns=rn)

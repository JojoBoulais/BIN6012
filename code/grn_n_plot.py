...
def main(argv):
    ...
    for p, pathway in enumerate(pathways):
        ...
        print(f"Processing pathway {p+1}/{len(pathways)+1}. ({pathway.get_name_as_path()})")
        ...
        grn_inferer = GNInfer(
            ...
            head_agg="mean",
            filtration="none",
            genelist=genes, #gene_list, # processing only these gene with how="some"
            num_genes=0,
        ...
        for disease in cell_diseases:
            ...
            if len(genes_present) < 2:
                print("Skipping pathway (not enough genes present)")
                continue

            genes_present = [gene for gene in genes_present \
                                    if gene in rn.keys()]

            mat = grn_for_plot.grn.loc[genes_present, genes_present].rename(index=rn, columns=rn)

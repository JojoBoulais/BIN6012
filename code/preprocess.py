...
def main(argv):
    ...
    if argv.organism:
        print(
            f"Injecting {argv.organism} as adata.obs['organism_ontology_term_id'] value.")
    ...
    if argv.n_top_genes > 0:
        ...
        scanpy.pp.highly_variable_genes(
            preprocessed_adata, n_top_genes=argv.n_top_genes)

    # https://scanpy.readthedocs.io/en/stable/api/generated/scanpy.pp.neighbors.html
    if argv.n_neighbors > 0:
        print(
            f"scanpy.pp.neighbors on dataset ({str(argv.n_neighbors)} neighbors).")
    ...
    while (not out_filepath):
        pfp = Path(fp)
        _out_filepath = str(pfp.with_name(
            pfp.stem + f"_preprocessed_{subset}.h5ad"))
